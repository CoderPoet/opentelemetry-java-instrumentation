...
java {
    toolchain {
        languageVersion.set(otelJava.minJavaVersionSupported.map {
...
val resourceClassesCsv = listOf(
    "Host",
    "Os",
    "Process",
    "ProcessRuntime"
).map { "io.opentelemetry.sdk.extension.resources.${it}ResourceProvider" }.joinToString(",")
tasks.withType<Test>().configureEach {
    ...
    jvmArgs(
        "-Dio.opentelemetry.javaagent.shaded.io.opentelemetry.context.enableStrictContext=${
    ...
    tasks.withType<Test>().configureEach {
        ...
        if (plugins.hasPlugin("org.unbroken-dome.test-sets") && configurations.findByName("latestDepTestRuntime") != null) {
            doFirst {
                ...
                val latestTestArtifacts =
                    configurations.getByName("latestDepTestRuntimeClasspath").resolvedConfiguration.resolvedArtifacts
...
configurations.configureEach {
    resolutionStrategy {
        ...
        dependencySubstitution {
            ...
            substitute(module("io.opentelemetry.instrumentation:opentelemetry-instrumentation-api-semconv")).using(
                project(":instrumentation-api-semconv")
            )
            substitute(module("io.opentelemetry.instrumentation:opentelemetry-instrumentation-annotations")).using(
                project(":instrumentation-annotations")
            )
            ...
            substitute(module("io.opentelemetry.instrumentation:opentelemetry-instrumentation-appender-api-internal")).using(
                project(":instrumentation-appender-api-internal")
            )
